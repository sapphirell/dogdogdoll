      ...
      >
        ...
        <view class="meta">
          ...
          <view class="row msg">
            ...
            <view v-if="s.unread > 0" class="badge">
...
/** WebSocket 状态提示小字（用于标题栏） */
const wsHintText = computed(() => {
  const s = wsStatus.value
  if (s === 'open') return ''
  if (s === 'connecting' || s === 'reconnecting') return '重连中…'
  if (s === 'error') return '连接异常，正在重试…'
  if (s === 'idle' || s === 'closed') return '连接已断开，正在重试…'
  return '重连中…'
})

/** 根据 wsHintText 更新系统导航标题 */
function applyNavTitle () {
  const sub = wsHintText.value
  const title = sub ? `聊天列表（${sub}）` : '聊天列表'
...
function pickSessionsPayload (res) {
  ...
  if (Array.isArray(res?.list)) {
    return {
      list: res.list,
      total: Number(res?.total || res.list.length || 0)
    }
  }
...
async function fetchUserSnapshot (uid) {
  ...
  try {
    ...
    if (res?.data?.status === 'success' && d) {
      return {
        name: d?.username || d?.user_name || '用户',
        avatar: d?.avatar || ''
      }
    }
  } catch (_) {}
  ...
  if (ev === 'message') {
    ...
    if (idx >= 0) {
      ...
      const next = {
        ...
        last_msg_preview: preview,
        last_msg_time: msgTime,
        unread: (from && from !== selfId.value)
          ? Math.min(999, Number(row.unread || 0) + 1)
          : row.unread
      }
...
onUnmounted(() => {
  ...
  // 如需离开就断开 WS，可以在这里调用 closeIM()
})

/** 监听 WS 状态变化，实时更新标题栏小字 */
watch(wsHintText, () => {
  applyNavTitle()
})
