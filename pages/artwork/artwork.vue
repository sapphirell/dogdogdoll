<template>
	<view-logs />
	<zhouWei-navBar type="transparentFixed" :backState="2000" :homeState="2000" :scrollTop="scrollTop" fontColor="#000"
		transparentFixedFontColor="#000" :titleCenter="true">
		<!-- 左侧 -->
		<template #left>
		  <view class="nav-left">
		    <view class="nav-back-pill" @click="goBack" aria-label="返回">
		      <image src="/static/artist/left.png" mode="aspectFill" style="width: 18rpx;height: 18rpx;"></image>
		    </view>
	...
	<view>
		...
		<view class="goods-section" v-if="detailData.goods_id && goodsInfo">
			...
			<view class="goods-item" @click="navigateToGoods(detailData.goods_id)">
				<view class="image-container">
					<image :src="goodsInfo.goods_images?.[0] || '/static/goods-default.png'" mode="aspectFill"
						class="goods-image" />
				</view>
		...
		<view style="padding: 10px;">
			<comment-list ref="commentListRef" :type="commentType" :relation-id="parseInt(pageId)"
...
<script setup>
	...
	// ① 让 transparentFixed 生效：把页面滚动量传给导航
	onPageScroll(e => {
	...
	// ④（可选）配置原生分享内容（仅小程序端）
	onShareAppMessage(() => ({
	  title: detailData.value.title || (brandInfo.value?.brand_name ? `${brandInfo.value.brand_name} · 作品` : "BJD作品"),
	...
	// ⑤ 点赞按钮的“安全距离”——取状态栏高度做 top 偏移
	const statusBarHeight = uni.getSystemInfoSync()?.statusBarHeight || 0
	const heartTopPx = `${statusBarHeight + 12}px`   // 比状态栏再下移 12px
...
<style lang="less" scoped>
	...
	.heart{
	  ...
	  top: 100rpx;
	  right: 20px;                              /* 默认右边距 */
	  right: calc(20px + constant(safe-area-inset-right)); /* iOS 旧 */
	  right: calc(20px + env(safe-area-inset-right));      /* iOS 新 */
	  z-index: 10;
