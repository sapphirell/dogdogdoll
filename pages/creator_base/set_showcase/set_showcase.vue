<template>
  <view class="artist-container">
    ...
    <!-- 妆图列表（默认显示） -->
    <view v-if="activeTab === 'faceup'" class="faceup-tab">
      <scroll-view class="faceup-scroll" scroll-y>
        <view class="faceup-list">
          ...
          >
            <image :src="item.image_urls[0]" mode="aspectFill" class="card-cover"></image>
            <view class="card-body">
              <view class="card-title">{{ item.head_name }}</view>

              <view class="faceup-tags" v-if="item.style_tags && item.style_tags.length">
                <view 
            ...
            <uni-icons class="card-arrow" type="arrowright" size="20" color="#b8c1cc"></uni-icons>
          </view>
        </view>
      </scroll-view>
...
const updateArtistHighlights = async () => {
  try {
    ...
    if (res.data.status !== 'success') {
      console.error('更新置顶展示图失败:', res.data.msg);
    }
    fetchArtistInfo();
  } catch (error) {
    console.error('更新置顶展示图失败:', error);
  }
};
