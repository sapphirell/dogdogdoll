        ...
        <scroll-view scroll-y class="form-container">
          ...
          <view class="form-group">
            ...
            <picker
              ...
              :value="getOrderTypeIndex(formData.order_type)"
              :range="orderTypes"
              range-key="text"
            >
              <view class="picker">
              ...
              <view v-for="(tier, index) in formData.order_config.tiers" :key="index" class="tier-item">
                <view class="tier-header">
                  <uni-icons
                    ...
                    v-if="formData.order_config.tiers.length > 1"
                  ></uni-icons>
                </view>
              ...
              <view v-for="(addon, index) in formData.order_config.addons" :key="index" class="tier-item">
                <view class="tier-header">
                  ...
                  <uni-icons
                    ...
                    v-if="formData.order_config.addons.length > 1"
                  ></uni-icons>
                </view>
                <view class="tier-inputs">
...
const onTierPickerChange = (e) => {
  ...
  if (option.value === 'blank') {
    ...
    formData.value.order_config.tiers.push({ ...option.value, price: parseFloat(option.value.price) || 0 })
  }
}
...
const onAddonPickerChange = (e) => {
  ...
  if (option.value === 'blank') {
    ...
    formData.value.order_config.addons.push({ ...option.value, price: parseFloat(option.value.price) || 0 })
  }
}
...
const fetchPlanList = async (reset = false) => {
  ...
  try {
    const res = await uni.request({
      ...
      method: 'GET',
      data: { page: currentPage.value, page_size: pageSize.value, status: activeTab.value },
      header: { Authorization: getAuthorization() }
    })
...
const submitPlan = async () => {
  if (!formData.value.artist_name) return uni.showToast({ title: '请填写计划名称', icon: 'none' })
  if (
  ...
  if (closeTime <= openTime) return uni.showToast({ title: '结束时间必须晚于开始时间', icon: 'none' })
...
// 添加/删除档位与加购
const addTier = () => formData.value.order_config.tiers.push({ title: '', price: 0, description: '' })
const removeTier = (index) => {
  if (formData.value.order_config.tiers.length > 1) formData.value.order_config.tiers.splice(index, 1)
}
const addAddon = () => formData.value.order_config.addons.push({ title: '', price: 0, description: '' })
...
const getOrderTypeText = (type) => {
  const map = { 1: '长期接单', 2: '限时手速', 3: '限时抽选', 4: '限时黑箱卡', 9: '关闭投递' }
